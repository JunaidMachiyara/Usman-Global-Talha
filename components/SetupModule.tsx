...
const SetupModule: React.FC<SetupModuleProps> = ({ userProfile, isModalMode = false, modalTarget, onModalClose, onModalSave, initialSection }) => {
    ...
    const crudComponents = useMemo(() => ({
        ...
        inventory: [
            { title: 'Items', entityName: 'items', data: state.items, columns: [{ key: 'id', header: 'Code' }, { key: 'name', header: 'Name' }, { key: 'categoryId', header: 'Category', render: (item: Item) => state.categories.find(c => c.id === item.categoryId)?.name || '' }, { key: 'avgProductionPrice', header: 'Avg Prod Price' }, { key: 'avgSalesPrice', header: 'Avg Sales Price' }], fields: [{ key: 'id', label: 'Code (Auto if empty)', type: 'text' }, { key: 'name', label: 'Name', type: 'text', required: true }, { key: 'categoryId', label: 'Category', type: 'select', options: state.categories.map(c => ({ value: c.id, label: c.name })), required: true }, { key: 'sectionId', label: 'Section', type: 'select', options: state.sections.map(s => ({ value: s.id, label: s.name })) }, { key: 'packingType', label: 'Packing Type', type: 'select', options: Object.values(PackingType).map(pt => ({ value: pt, label: pt })), required: true }, { key: 'baleSize', label: 'Packing Size (Kg/Unit)', type: 'number' }, { key: 'packingColor', label: 'Packing Color', type: 'text' }, { key: 'avgProductionPrice', label: 'Average Production Price ($/Kg)', type: 'number', required: true }, { key: 'avgSalesPrice', label: 'Average Sales Price ($/Kg)', type: 'number' }, { key: 'demandFactor', label: 'Demand Factor (1-10)', type: 'number' }, { key: 'openingStock', label: 'Opening Stock (Units)', type: 'number' }], idGenerator: generateItemId, initialState: { id: '', name: '', categoryId: '', packingType: PackingType.Bales, baleSize: 0, packingColor: '', avgProductionPrice: 0, avgSalesPrice: 0, demandFactor: 5 } as any, icon: Icons.cube },

...
const CrudManager = <T extends Entity & { id: string, name?: string, fullName?: string, accountTitle?: string, plateNumber?: string, startingBalance?: number, openingStock?: number, onDuty?: boolean, offDutyStatus?: 'Holidays' | 'Fired', currency?: Currency, defaultCurrency?: Currency },>({ title, data, columns, fields, idGenerator, entityName, initialState, showNotification, state, isOpen, onOpen, onClose, onSaveSuccess, onOpenImportModal, userProfile, icon, isInitiallyExpanded = false }: CrudManagerProps<T>) => {
    ...
    const handleSave = () => {
        ...
        if (isEditing) {
            ...
            // Allow manual ID entry if provided, otherwise generate.
            // Also checks for duplicates if ID is manually entered.
            if ((newItem as any).id) {
                 if (data.some(d => d.id === (newItem as any).id)) {
                    setError(`ID '${(newItem as any).id}' already exists.`);
                    return;
                }
            } else if (idGenerator) {
                                ...
                                <React.Fragment key={String(field.key)}>
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-slate-700 mb-1">
                                            ...
                                            {/* Conditionally required for baleSize when not Kg */}
                                            {isBaleSizeFieldForItems && !isItemBaleSizeDisabled && <span className="text-red-500 ml-1">*</span>}
                                        </label>
                                        {field.type === 'select' ? (
                                            <select
...
const Icons = {
    ...
    'clipboard-list': <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>,
...
const SetupModule: React.FC<SetupModuleProps> = ({ userProfile, isModalMode = false, modalTarget, onModalClose, onModalSave, initialSection }) => {
    ...
    const crudComponents = useMemo(() => ({
        ...
        inventory: [
            { title: 'Items', entityName: 'items', data: state.items, columns: [{ key: 'id', header: 'Code' }, { key: 'name', header: 'Name' }, { key: 'categoryId', header: 'Category', render: (item: Item) => state.categories.find(c => c.id === item.categoryId)?.name || '' }, { key: 'avgProductionPrice', header: 'Avg Prod Price' }, { key: 'avgSalesPrice', header: 'Avg Sales Price' }], fields: [{ key: 'id', label: 'Code (Auto if empty)', type: 'text' }, { key: 'name', label: 'Name', type: 'text', required: true }, { key: 'categoryId', label: 'Category', type: 'select', options: state.categories.map(c => ({ value: c.id, label: c.name })), required: true }, { key: 'sectionId', label: 'Section', type: 'select', options: state.sections.map(s => ({ value: s.id, label: s.name })) }, { key: 'packingType', label: 'Packing Type', type: 'select', options: Object.values(PackingType).map(pt => ({ value: pt, label: pt })), required: true }, { key: 'baleSize', label: 'Packing Size (Kg/Unit) - Required for Bales/Sacks', type: 'number' }, { key: 'packingColor', label: 'Packing Color', type: 'text' }, { key: 'avgProductionPrice', label: 'Average Production Price ($/Kg)', type: 'number', required: true }, { key: 'avgSalesPrice', label: 'Average Sales Price ($/Kg)', type: 'number' }, { key: 'demandFactor', label: 'Demand Factor (1-10)', type: 'number' }, { key: 'openingStock', label: 'Opening Stock (Units)', type: 'number' }], idGenerator: generateItemId, initialState: { id: '', name: '', categoryId: '', packingType: PackingType.Bales, baleSize: 0, packingColor: '', avgProductionPrice: 0, avgSalesPrice: 0, demandFactor: 5 } as any, icon: Icons.cube },
            ...
            { title: 'Categories', entityName: 'categories', data: state.categories, columns: [{ key: 'id', header: 'ID' }, { key: 'name', header: 'Name' }], fields: [{ key: 'id', label: 'ID (Auto if empty)', type: 'text' }, { key: 'name', label: 'Name', type: 'text', required: true }], idGenerator: generateCategoryId, initialState: { id: '', name: '' } as any, icon: Icons.tag },
            { title: 'Sections', entityName: 'sections', data: state.sections, columns: [{ key: 'id', header: 'ID' }, { key: 'name', header: 'Name' }], fields: [{ key: 'id', label: 'ID (Auto if empty)', type: 'text' }, { key: 'name', label: 'Name', type: 'text', required: true }], idGenerator: generateSectionId, initialState: { id: '', name: '' } as any, icon: Icons['view-grid'] },
        ],
        structure: [
